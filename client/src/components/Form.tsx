...
export default function Form({form, handleChange, handleSubmit, editingId}: FormProps  ){
    return  (
    <div>
      ...
      <button className="bg-black w-full text-white p-3 rounded-lg" disabled={form.title.trim() === "" || form.description.trim() === ""}>{ editingId === null ? "Create Ticket": "Update Ticket"}  </button> 
      </form>
      </div>
      )
}